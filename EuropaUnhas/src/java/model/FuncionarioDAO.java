...
public class FuncionarioDAO extends DataBaseDAO {
    ...
    public ArrayList<Funcionario> getListaFun() throws Exception{
        ArrayList<Funcionario> lista = new ArrayList<Funcionario>();
        String sql = "SELECT f.* from funcionario as f INNER JOIN usuario as u on u.idusuario = f.idusuario WHERE u.idperfil=3  and u.status = 1";
        this.conectar();
        Statement stm = conn.createStatement();
        ResultSet rs = stm.executeQuery(sql);
        while(rs.next()){
