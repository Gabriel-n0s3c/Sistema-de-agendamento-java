...
public class ClienteDAO extends DataBaseDAO{
    ...
    public Cliente getCarregaPorUsuario(int idUsuario) throws Exception{
        ...
        if(rs.wasNull()){
            ...
            c.setEndereco(rs.getString("endereco"));
            c.setTelefone(rs.getString("telefone"));
            c.setUsuario(new Usuario().getCarregaPorID(idUsuario));
        }
        this.desconectar();
        return c;
    ...
    public ArrayList<Integer> getQuantidadePorStatus() throws Exception {
        ...
        String sql = "SELECT "
                + "count(case when u.status = 2 then 1 else null end) as desativado,"
                + "count(case when u.status = 1 then 1 else null end) as ativo "
                + "FROM usuario as u INNER JOIN cliente as c on c.idusuario = u.idusuario"
                + " WHERE u.idperfil = 4";
        this.conectar();
        Statement stm = conn.createStatement();
        ResultSet rs = stm.executeQuery(sql);
        if (rs.next()) {
            int desativado = rs.getInt("desativado");
            int ativo = rs.getInt("ativo");
            lista.add(desativado);
            lista.add(ativo);
