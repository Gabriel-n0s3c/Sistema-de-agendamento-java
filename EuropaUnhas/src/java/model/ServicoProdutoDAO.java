...
public class ServicoProdutoDAO extends DataBaseDAO {
    ...
    public boolean descontaQuantidadeProdutos(int idservico) {
        try {
            String sql = "UPDATE produto as p "
                    + "INNER JOIN servico_produto as ps on p.idproduto = ps.idproduto "
                    + "INNER JOIN servico as s on s.idservico = ps.idservico "
                    + "set p.quantidade = (p.quantidade - ps.descontarqtd) "
                    + "where ps.idservico = ?";
            this.conectar();
            pstm = conn.prepareStatement(sql);
            pstm.setInt(1, idservico);
    ...
    public boolean somaQuantidadeProdutos(int idservico) {
        try {
            String sql = "UPDATE produto as p "
                    + "INNER JOIN servico_produto as ps on p.idproduto = ps.idproduto "
                    + "INNER JOIN servico as s on s.idservico = ps.idservico "
